...
import sendEmail from "../manually_def_functions/nodemailer.js";
...
async function creds_compare(req , res){
    try {
        ...
        //   // two step verification part starts here
        //   let unhashedtoken = await crypto.randomBytes(10).toString('hex');
        //   // saving token to db
        //   let waiter2 = await USERreference.findByIdAndUpdate({_id : userdetails._id} , {unhashedtoken : unhashedtoken})


        //   // mail se send kar denge http://localhost:3000/two_step_verify/unhashedtoken
        //   sendEmail( userdetails.email , "a new login request has been made" , "click on link to send your response" , `http://localhost:3000/two_step_verify/${unhashedtoken}` )
        //   console.log( 'mail sent where id is ' , unhashedtoken )
...
async function set_jwt(req , res){
  ...
  sendEmail( userdetail.email , "new device logged in to translator.com" , "a new device has been logged in to your account " )
